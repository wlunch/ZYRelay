...
class CodeExampleDetector:
    def inline(
        ...
        for pattern, target, kind in (
            (rf"(?:正确示例|推荐写法|正例|例如)[:：]?\s*[`“\"]?({TOKEN})", positive, "positive"),
            (rf"(?:错误示例|反例|禁止使用|错误写法)[:：]?\s*[`“\"]?({TOKEN})", negative, "negative"),
        ):
    ...
    def from_block(
        ...
        *,
        ...
        code_like = bool(
            ...
            or re.search(r"(?:\bclass\b|\bdef\b|\bpublic\b|\bprivate\b|\bimport\b|\breturn\b|[{};])", text)
        )
        if not code_like:
            return None
        return CodeExample(
