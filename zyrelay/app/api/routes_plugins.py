...
def validate_request(
    plugin_id: str, value: PluginRequest, request: Request
):
...
def execute(plugin_id: str, value: PluginRequest, request: Request):
    return _response(
        _plugin(request, plugin_id).execute(_http_request(value))
    )
...
@router.get(
    "/{plugin_id}/executions/{execution_id}/artifacts/{artifact_id}"
)
def get_artifact(
    plugin_id: str,
    execution_id: str,
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="{reference.file_name}"'
            )
        },
    )
