...
from benchmark.scripts.common import (
    CONFIG_ROOT, json_dump, now_utc, sha256_file, source_download_path,
    source_metadata_path, validate_source_url, load_yaml,
)
...
def download_source(source: dict, max_bytes: int, force: bool = False) -> dict:
    ...
    request = Request(source["source_url"], headers={"User-Agent": "ZYRelay-Benchmark/1.0"})
    ...
    try:
        ...
        with response:
            ...
            metadata = {
                ...
                "downloaded_at": now_utc(), "content_type": content_type,
                "original_file_name": Path(response.geturl()).name or target.name,
                "file_size": target.stat().st_size, "sha256": sha256_file(target),
                "download_method": "urllib_https_whitelist",
            }
            json_dump(source_metadata_path(source), metadata)
            return {"source_id": source["source_id"], "status": "downloaded", "path": str(target), **metadata}
        ...
        try:
            ...
            completed = subprocess.run(
                ["curl", "--fail", "--location", "--silent", "--show-error", "--proto", "=https", "--max-filesize", str(max_bytes), "--output", str(temp_path), source["source_url"]],
            ...
            temp_path.replace(target)
            metadata = {"source_id": source["source_id"], "source_url": source["source_url"], "downloaded_at": now_utc(), "content_type": "unknown", "original_file_name": target.name, "file_size": target.stat().st_size, "sha256": sha256_file(target), "download_method": "curl_https_whitelist_fallback"}
            json_dump(source_metadata_path(source), metadata)
            return {"source_id": source["source_id"], "status": "downloaded", "path": str(target), **metadata}
        except (OSError, ValueError, subprocess.TimeoutExpired) as fallback_error:
            if temp_path:
                temp_path.unlink(missing_ok=True)
            return {"source_id": source["source_id"], "status": "failed", "error": f"{exc}; fallback={fallback_error}"}
...
def main() -> int:
    ...
    selected = [item for item in config["sources"] if item.get("enabled") and (not args.source_id or item["source_id"] == args.source_id)]
    results = [download_source(item, int(config.get("max_download_bytes", 25 * 1024 * 1024)), args.force) for item in selected]
