...
def create_scanned_pdf(
    source: Path, target: Path, quality: str = "clean", max_pages: int | None = None
) -> dict:
    ...
    for index, page in enumerate(original):
        ...
        pixmap = page.get_pixmap(
            matrix=fitz.Matrix(dpi / 72, dpi / 72), colorspace=fitz.csRGB, alpha=False
        )
        ...
        dimensions.append(
            {"page_no": index + 1, "width": pixmap.width, "height": pixmap.height}
        )
    original.close()
    if not dimensions:
        output.close()
    ...
    return {
        ...
        "tool": f"PyMuPDF {fitz.VersionBind}",
        "extractable_text_length": len(extracted.strip()),
    }
...
def main() -> int:
    ...
    metadata = create_scanned_pdf(
        args.source, args.target, args.quality, args.max_pages
    )
