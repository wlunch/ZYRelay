...
def convert_source(source: dict, force: bool = False) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="zyrelay-html-pdf-") as directory:
        ...
        if completed.returncode != 0 or not output.exists():
            return {
                ...
                "status": "failed",
                "error": (
                    completed.stderr or completed.stdout or "pdf_conversion_failed"
                )[-1000:],
            }
    ...
    metadata = {
        ...
        "conversion_tool": "LibreOffice soffice writer_pdf_Export",
        "sha256": sha256_file(target),
        "file_size": target.stat().st_size,
    }
    previous = source_metadata_path(source)
    current = (
        __import__("json").loads(previous.read_text(encoding="utf-8"))
        if previous.exists()
        else {}
    )
...
def main() -> int:
    ...
    selected = [
        s
        for s in load_yaml(CONFIG_ROOT / "sources.yaml")["sources"]
        if s.get("enabled") and (not args.source_id or s["source_id"] == args.source_id)
    ]
